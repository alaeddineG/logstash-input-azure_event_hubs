...
NOTE: Threads are currently available only as a global setting across all event hubs in a single `medfar_logs`
input definition. However if your configuration includes multiple `medfar_logs` inputs, the threads setting applies
