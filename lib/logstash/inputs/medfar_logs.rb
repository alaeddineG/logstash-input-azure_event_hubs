# encoding: utf-8
require "logstash-input-medfar_logs"
...
class LogStash::Inputs::MedfarLogs < LogStash::Inputs::Base
  ...
  # [@metadata][medfar_logs][name] - the name of hte event host
  # [@metadata][medfar_logs][consumer_group] - the consumer group that consumed this event
  # [@metadata][medfar_logs][processor_host] - the unique identifier that identifies which host processed this event. Note - there can be multiple processor hosts on a single instance of Logstash.
  # [@metadata][medfar_logs][partition] - the partition from which event came from
  # [@metadata][medfar_logs][offset] - the event hub offset for this event
  # [@metadata][medfar_logs][sequence] - the event hub sequence for this event
  # [@metadata][medfar_logs][timestamp] - the enqueued time of the event
  # [@metadata][medfar_logs][event_size] - the size of the event
  # basic Example:
  # medfar_logs {
  #    config_mode => "basic"
  #    event_hub_connections => ["Endpoint=sb://example1...;EntityPath=event_hub_name1"]
  ...
  def run(queue)
    event_hub_threads = []
    named_thread_factory = LogStash::Inputs::Azure::NamedThreadFactory.new("medfar_logs-worker", @id)
